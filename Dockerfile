...
RUN --mount=type=secret,id=github-token ls /run/secrets
RUN --mount=type=secret,id=github-token md5sum <(cat /run/secrets/github_token)
...
RUN --mount=type=secret,id=github-token NODE_AUTH_TOKEN=$(cat /run/secrets/github-token) npm ci
