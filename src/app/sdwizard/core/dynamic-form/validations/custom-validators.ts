...
export class CustomValidators {
  ...
  static lessThanValidator(fieldId: string): ValidatorFn {
  ...
  static uriPatternValidator(): ValidatorFn {
    return (control: AbstractControl): { [key: string]: boolean } | null => {
      ...
      if (control.value && !uriRegex.test(control.value)) {
        return {uriPattern: control.value};
      }
      return null;
    };
  }

  static urlPatternValidator(): ValidatorFn {
    return (control: AbstractControl): { [key: string]: boolean } | null => {
      const urlRegex = /[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]+)?/gi;
      if (control.value && !urlRegex.test(control.value)) {
        return {urlPattern: control.value};
      }
      return null;
    };
  }
  ...
  static checkChildrenValidity(): ValidatorFn {
    return (control: AbstractControl): { [key: string]: boolean } | null => {
      ...
      if (fg instanceof AbstractControl) {
        ...
        return {childrenValid: false};
      }
    };
  }

  static checkControl(control: AbstractControl) {
    if (control instanceof FormArray) {
      if (control.value.every((value: string) => value !== null && value !== '')) {
        return true;
      }
